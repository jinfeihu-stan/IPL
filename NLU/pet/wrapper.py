...
#from tensorboardX import SummaryWriter  # 进行可视化
from torch.utils.data import RandomSampler, DataLoader, SequentialSampler
from tqdm import trange, tqdm
from transformers import InputExample, AdamW, get_linear_schedule_with_warmup, \
    BertForMaskedLM,BertConfig, BertTokenizer, RobertaConfig, \
    RobertaTokenizer, AlbertForMaskedLM, AlbertTokenizer, AlbertConfig, GPT2Config, \
...
from pet.config import WrapperConfig, EvalConfig, TrainConfig
from pet.utils import InputFeatures, DictDataset, distillation_loss, exact_match,trim_input_ids
...
class ContinuousPrompt(torch.nn.Module):
    def __init__(self, config: WrapperConfig, tokenizer):
        ...
        self.prompt_embeddings = torch.nn.Embedding(self.config.prompt_length, config.embedding_dim)
        ...
        self.prompt_query = torch.nn.Linear(config.embedding_dim, config.prompt_head_size)
        # self.prompt_query.cuda()
        self.prompt_key = torch.nn.Linear(config.embedding_dim, config.prompt_head_size)
        # self.prompt_key.cuda()
...
class TransformerModelWrapper:
    ...
    def save(self, path: str) -> None:
        ...
        if self.config.prompt_encoder_type == "lstm":
            ...
            state = {
                ...
                "prompt_query": model_to_save.prompt_query.state_dict(),
                "prompt_key": model_to_save.prompt_key.state_dict()
            }
        else:
            raise ValueError("unknown prompt_encoder_type.")
    ...
    def from_pretrained(cls, path: str) -> 'TransformerModelWrapper':
        ...
        if "prompt_query" in data:
            wrapper.model.prompt_query.load_state_dict(data["prompt_query"])
        if "prompt_key" in data:
            wrapper.model.prompt_key.load_state_dict(data["prompt_key"])
    ...
    def train(self,
        ...
        if self.config.prompt_encoder_type == "mlp":
            embedding_parameters = [
                {'params': [p for p in cur_model.mlp.parameters()]},
                {'params': [p for p in cur_model.prompt_embeddings.parameters()]},
                {'params': [p for p in cur_model.prompt_query.parameters()]},
                {'params': [p for p in cur_model.prompt_key.parameters()]}
            ]
        else:
            embedding_parameters = [
                {'params': [p for p in cur_model.prompt_embeddings.parameters()]},
        ...
        for _ in train_iterator:
            ...
            for step, batch in enumerate(epoch_iterator):
                ...
                if (step + 1) % gradient_accumulation_steps == 0:
                    ...
                    if logging_steps > 0 and global_step % logging_steps == 0:
                        ...
                        if not self.config.prompt_tuning:
                            ...
                            learning_rate_scalar = embedding_scheduler.get_lr()[0]
                    ...
                    if global_step % self.config.eval_every_step == 0:
                        ...
                        if self.config.task_name in ["cb", "record"]:
                            f1_str = "f1" if self.config.task_name != "cb" else "f1-macro"
                            if dev_scores[f1_str] >= best_dev_f1:
                        ...
                        elif self.config.task_name in ["multirc"]:
                            if dev_scores["em"] >= best_dev_em:
                                ...
                                best_dev_em = dev_scores["em"]
                                best_global_step = global_step
                                best_loss = tr_loss

                                logger.info("Saving trained model at {}...".format(pattern_iter_output_dir))
                                logger.info("best_dev_f1: %.4f | best_dev_em: %.4f | best_global_step: %d" % \
                                            (best_dev_f1, best_dev_em, best_global_step))
                                logger.info(dev_scores)
                ...
                if 0 < max_steps < global_step or early_stop_epoch >= 20:
    ...
    def eval(self,
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                eval_loss = nn.CrossEntropyLoss()(prediction_scores.view(-1, len(self.config.label_list)),
                                                  labels.view(-1))
    ...
    def generate_default_inputs_without_mutual(self,batch: Dict[str, torch.Tensor])-> Dict[str, torch.Tensor]:
        input_ids = batch['input_ids']
        bz, seq_length = batch['input_ids'].shape
        ...
        prompt_id_tensor = torch.LongTensor(list(range(self.config.prompt_length))).cuda()
        prompt_id_tensor = prompt_id_tensor.unsqueeze(dim=0).expand(bz, prompt_id_tensor.size(-1))
        prompt_embeds = model.prompt_embeddings(prompt_id_tensor)  # [batch_size, prompt_length, embed_size]
        ...
        if self.config.prompt_encoder_type == "lstm":
            ...
            raise ValueError("unknown prompt_encoder_type.")
        inputs_embeds = torch.cat([prompt_embeds, raw_embeds], dim=1)
        # process attention mask
        attention_mask = torch.cat([torch.ones_like(prompt_id_tensor), batch['attention_mask']], dim=1)
    ...
    def generate_default_inputs(self, batch: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        if self.config.prompt_encoder_type == "lstm":
            ...
            raise ValueError("unknown prompt_encoder_type.")
        prompt_embeds = self.prompt_score(prompt_embeds, raw_embeds)
        # [batch_size, prompt_length + seq_length, embed_size]
        inputs_embeds = torch.cat([prompt_embeds, raw_embeds], dim=1)
        # process attention mask
        ...
        if self.config.model_type in ['bert']:
            token_type_ids = torch.cat([torch.ones_like(prompt_id_tensor), batch['token_type_ids']], dim=1)
            inputs['token_type_ids'] = token_type_ids

        return inputs

    def prompt_score(self, prompt_embed, input_embed):
        input_shape = list(input_embed.size())
        # print(input_embed.size())
        # print(prompt_embed.size())
        key_size = torch.reshape(input_embed, (-1, input_shape[-1]))
        # print(key_size.size())
        model = self.model.module if hasattr(self.model, 'module') else self.model
        key_w = model.prompt_key
        # key_w  = nn.Linear(input_shape[-1], head_size)
        key = key_w(key_size)
        # print(key.size())
        prompt_shape = list(prompt_embed.size())
        query_size = torch.reshape(prompt_embed, (-1, prompt_shape[-1]))
        query_w = model.prompt_query
        # query_w = nn.Linear(prompt_shape[-1], head_size)
        query = query_w(query_size)
        # print(query.size())
        head_size = list(query.size())[-1]
        Q = torch.reshape(query, (-1, prompt_shape[1], head_size))
        K = torch.transpose(torch.reshape(key, (-1, input_shape[1], head_size)), 1, 2)
        V = prompt_embed
        scores = torch.matmul(Q, K)
        scores = torch.mean(scores / math.sqrt(head_size), dim=-1)
    ...
    def mlm_train_step(self, labeled_batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        #inputs = self.generate_default_inputs_without_mutual(labeled_batch)
        mlm_labels, labels = labeled_batch['mlm_labels'], labeled_batch['labels']
        bs = list(mlm_labels.size())[0]
        seq = self.config.prompt_length
        prefix = -1 * torch.ones(bs, seq)
        prefix = torch.LongTensor(prefix.numpy()).cuda()
        mlm_labels = torch.cat([prefix, mlm_labels], dim=-1)
        outputs = self.model(**inputs)
        lm_logits = outputs[0]
        prediction_scores = self.preprocessor.pvp.convert_mlm_logits_to_cls_logits(mlm_labels, lm_logits)
    ...
    def mlm_eval_step(self, batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        #inputs = self.generate_default_inputs_without_mutual(batch)
        outputs = self.model(**inputs)
        ...
        prefix = torch.LongTensor(prefix.numpy()).cuda()
        mlm_labels = torch.cat([prefix, mlm_labels], dim=-1)
        return self.preprocessor.pvp.convert_mlm_logits_to_cls_logits(mlm_labels, lm_logits)
