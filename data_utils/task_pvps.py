...
class RtePVP(PVP):
    ...
    def get_parts(self, example: InputExample) -> FilledPattern:
        ...
        if self.pattern_id == 1:
            ...
            string_list_a = [text_a, '[SEP]', text_b, "?", "the", "answer:", self.mask]
            string_list_b = []
            return string_list_a, string_list_b
...
class CbPVP(PVP):
    ...
    def get_parts(self, example: InputExample) -> FilledPattern:
        ...
        if self.pattern_id == 1:
            #string_list_a = [text_a, "[SEP]", example.text_b, "?", 'the', " answer: ", self.mask]
            string_list_a = [text_a, "[SEP]", example.text_b, "?"," answer: ", self.mask,'.']
            string_list_b = []
            return string_list_a, string_list_b
    ...
    def get_parts(self, example: InputExample) -> FilledPattern:
        ...
        if self.pattern_id == 1:
            if question == "cause":
                ...
                string_list_a = [choice1, 'or', choice2, '?', premise, 'beacuse', self.mask * num_masks, '.']
                string_list_b = []
                return string_list_a, string_list_b

            elif question == "effect":

                string_list_a = [choice1, 'or', choice2, '?', premise, 'so', self.mask * num_masks, '.']
                string_list_b = []
                return string_list_a, string_list_b
...
class BoolQPVP(PVP):
    ...
    def get_parts(self, example: InputExample) -> FilledPattern:
        ...
        if self.pattern_id == 1:
            ...
            # string_list_a = [passage, '.', 'the', 'Question:', question, '?', 'the', 'Answer:', self.mask]
            string_list_a = [passage, '.', 'Question:', question, '?', 'Answer:', self.mask, '.']
            string_list_b = []
            return string_list_a, string_list_b
...
class MultiRcPVP(PVP):
    ...
    def get_parts(self, example: InputExample) -> FilledPattern:
        ...
        if self.pattern_id == 1:
            ...
            string_list_a = [passage, 'Question: ', question, '? Is "', answer, '" a correct answer?', self.mask, '.']
            string_list_b = []
            return string_list_a, string_list_b
        else:
            raise ValueError("unknown pattern_id.")
...
class WicPVP(PVP):
    ...
    def get_parts(self, example: InputExample) -> FilledPattern:
        ...
        if self.pattern_id == 1:
            ...
            string_list_a = [text_a, '[SEP]', text_b, "the", word, '?', "the", self.mask]  # 效果不好，可不用
            string_list_b = []
            return string_list_a, string_list_b
